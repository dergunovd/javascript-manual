...
### Унарные операторы
* `++` - инкремент. Допустимы и префиксная, так и постфиксная запись
```JavaScript
let a = 5;
a++; // a == 6;
++a; // a == 7;
```
* `--` - декремент. Допустимы и префиксная, так и постфиксная запись
```JavaScript
let a = 5;
a--; // a == 4;
--a; // a == 3;
```
* `+` - унарный плюс. Если применяется к числовому значению, то ничего с ним не делает. Если применяется к нечисловому значению - приводит его к числу подобно фунцкии `Nubmer()`
```JavaScript
